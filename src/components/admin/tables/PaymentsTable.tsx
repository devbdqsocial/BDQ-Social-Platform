...
import { PAYMENT_STATUS } from "@/lib/status-badges";
...
const columns: ColumnDef<Row>[] = [
  ...
  { id: "status", accessorFn: (r) => r.status, header: "Status", cell: ({ row }) => { const s = PAYMENT_STATUS[row.original.status]; return s ? <Badge variant={s.variant}>{s.label}</Badge> : row.original.status; } },
  { id: "date", accessorFn: (r) => r.createdAt.getTime(), header: "When", cell: ({ row }) => <span className="text-muted-foreground">{fmtDateTime(row.original.createdAt)}</span> },
];

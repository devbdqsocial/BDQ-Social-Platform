...
export function DesignerStatusBar() {
  const { cursorFt, scale, tool, measureLine, measureDist, selected, selectedIds, displayUnit, placing, plotOrigin } = useDesigner();
  return (
    <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 rounded-xl border border-border bg-card px-3 py-2 text-xs text-muted-foreground">
      <span className="tabular-nums">{cursorFt ? `x ${fmtLen(cursorFt[0] - plotOrigin[0], displayUnit)} · y ${fmtLen(cursorFt[1] - plotOrigin[1], displayUnit)}` : "—"}</span>

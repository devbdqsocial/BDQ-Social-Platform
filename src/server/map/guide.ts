...
export interface EventGuide {
  ...
  hasLayout: boolean;
  layout: RenderLayout; // booked stalls relabelled to brand names for the canvas
  brands: GuideBrand[];
...
export async function getEventGuide(): Promise<EventGuide | null> {
  ...
  const event = await db.event.findUnique({
    ...
    include: {
      ...
      stalls: {
        include: {
          bookings: {
            where: { status: "BOOKED", vendorProfileId: { not: null } },
            take: 1,
  ...
  const layoutV2 = event.mapLayout?.layoutJson ? upgradeLayout(event.mapLayout.layoutJson) : null;
  const zones: Zone[] = layoutV2?.zones ?? [];
  const canvas = layoutV2?.canvas ?? { widthFt: 230, heightFt: 160 };
  ...
  for (const s of event.stalls) {
    ...
    const zone = zones.length ? zoneOf({ id: s.id, kind: "stall", type: "STALL", label: s.label, xFt: s.xFt, yFt: s.yFt, widthFt: s.widthFt, heightFt: s.heightFt, rotation: s.rotation }, zones)?.name ?? null : null;
  ...
  const { layout } = stallsToRenderLayout(
    event.stalls.map((s) => ({
      kind: s.kind, status: s.status, label: brandByLabel.get(s.label)?.brandName ?? (s.kind === "INFRA" ? s.label : s.label),

...
| Reference | `reference/rpa/rpacomunicacion.com`, especially cursor, section color switching, shape masks, menu animation, footer motion, and type/grid system. |

## Reference Findings

The RPA reference is a complete interaction system, not only a style sample.
...
| Rich morphing menu shape and staggered labels. | Full-screen menu exists with label rise animation. | Menu lacks RPA-style shape panel, preview/media behavior, and cursor-safe layering. |
...
| P1 | Typography has multiple systems. | RPA tokens, Tailwind sizes, bracket rem sizes, admin Geist sizes. | Inconsistent polish and scaling behavior. |
...
| P1 | Utility/admin screens and marketing screens have different polish standards. | Admin uses shadcn/Geist; public/vendor use RPA. | Good separation, but shared quality rules are missing. |
...
1. Public/customer/vendor brand surfaces use the RPA-inspired editorial system.
